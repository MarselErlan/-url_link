...
async function loginWithGoogle() {
  try {
    updateDebugInfo('Starting Google OAuth login...');
    ...
    // In production, you need to get a real Google OAuth client ID
    ...
    updateDebugInfo('Opening Google login...');
    
    // Step 2: Launch web auth flow
    ...
    updateDebugInfo('Processing Google response...');
    ...
    updateDebugInfo('Exchanging code for token...');
    ...
    const tokenResponse = await fetch('https://oauth2.googleapis.com/token', {
      ...
      body: new URLSearchParams({
        client_id: clientId,
        client_secret: 'YOUR_CLIENT_SECRET', // You need to get this from Google Console
        code: code,
    ...
    updateDebugInfo('Getting user info from Google...');
    ...
    if (userInfo.email) {
      ...
      console.log('Google OAuth login successful:', userInfo.email);
    ...
    // Fallback to simple approach for testing
    console.log('Falling back to simple login...');
    const email = prompt('Google OAuth failed. Enter your email for testing:');
    if (email && email.includes('@')) {
      ...
      updateDebugInfo(`Logged in as: ${email} (fallback)`);
      
      console.log('Fallback login successful:', email);
      return email;
    } else {
