...
async function checkUrlStatus(url) {
  try {
    ...
    let apiUrl = `${API_BASE_URL}/api/status/${encodeURIComponent(url)}`;
    let headers = { 'x-user-id': userId };
    if (jobId) {
      // Optionally send jobId as a header for clarity (not required by backend, but can be used for debugging)
